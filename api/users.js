...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const rows = await sql`
      SELECT id, username, display_name, email, role, active, must_change_password, created_at,
             financial_status, amount_paid, amount_due, payment_due_date, billing_type, plan_name, plan_version, plan_terms
  ...
  const billingType = ["one_time", "monthly"].includes(req.body?.billingType) ? req.body.billingType : (current.billing_type || "one_time");
  ...
  try {
    const rows = await sql`
      UPDATE portal_users SET
        ...
        financial_status = ${financialStatus}, amount_paid = ${amountPaid}, amount_due = ${amountDue}, payment_due_date = ${paymentDueDate}, billing_type = ${billingType},
...
function serialize(row) {
  return {
    ...
    billingType: row.billing_type === "monthly" ? "monthly" : "one_time",
